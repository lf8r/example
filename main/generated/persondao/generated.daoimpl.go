...
// PersonClientDBImpl implements PersonClient.
type PersonClientDBImpl struct {
	Tx *sql.Tx
}

var _ PersonClient = (*PersonClientDBImpl)(nil)

// Create creates a new data.Person instance.
func (c *PersonClientDBImpl) Create(value *data.Person) (*data.Person, error) {
	...
	if _, err := c.Tx.Exec("insert into Person values($1,$2,$3,$4,$5)", value.ID, value.Name, value.Created.Time, value.Modified.Time, string(content)); err != nil {
...
// Delete deletes the given data.Person instance.
func (c *PersonClientDBImpl) Delete(value *data.Person) error {
	if value.ID == "" {
		return fmt.Errorf("missing id")
	}

	if _, err := c.Tx.Exec("delete from Person where ID=$1", value.ID); err != nil {
...
// DeleteByID deletes the data.Person with the given ID, returning without error
// if the ID doesn't exist.
func (c *PersonClientDBImpl) DeleteByID(id string) error {
	if id == "" {
		return fmt.Errorf("missing id")
	}

	if _, err := c.Tx.Exec("delete from Person where ID=$1", id); err != nil {
		return fmt.Errorf("delete: %w", err)
	}
...
// GetByID gets the data.Person with the given id.
func (c *PersonClientDBImpl) GetByID(id string) (*data.Person, error) {
	if id == "" {
		return nil, fmt.Errorf("missing id")
	}

	rows, err := c.Tx.Query("select * from Person where ID=$1", id)
	if err != nil {
		return nil, fmt.Errorf("select * from Person with id %s: %w", id, err)
	}
...
// List returns a slice of data.Person instances fulfilling the query (if given).
...
//  m["Name"] = v.Name
//  m["Address"] = map[string]string{"Street1": v.Address.Street1}
func (c *PersonClientDBImpl) List(query map[string]interface{}) ([]data.Person, error) {
...
// Update updates the given data.Person instance.
func (c *PersonClientDBImpl) Update(value *data.Person) (*data.Person, error) {
	...
	if _, err = c.Tx.Exec("Update Person set Name=$2, Created=$3, Modified=$4, Data=$5 where ID=$1", value.ID, value.Name, value.Created.Time, time.Now(), string(content)); err != nil {
...
// PartialUpdate applies changes represented by values to the data.Person with the
// given id.
func (c *PersonClientDBImpl) PartialUpdate(id string, values map[string]interface{}) (*data.Person, error) {
