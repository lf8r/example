...
func Handler(w http.ResponseWriter, r *http.Request) {
	...
	ctx, err := persondao.BeginTx(r.Context())
	if err != nil {
		w.Header().Add("Content-Type", "application/text")
	...
	if dtype == persondao.PersonTypeID {
		handlePerson(w, r)
	}
...
// handlePerson handles requests for Person.
func handlePerson(w http.ResponseWriter, r *http.Request) {
	c := persondao.PersonService(r.Context())

	switch r.Method {
	case http.MethodGet:
		handlePersonGet(c, w, r)

		return

	case http.MethodPost:
		handlePersonPost(c, w, r)

		return

	case http.MethodPut:
		handlePersonPut(c, w, r)

		return

	case http.MethodDelete:
		handlePersonDelete(c, w, r)

		return

	case http.MethodPatch:
		handlePersonPatch(c, w, r)
...
// handlePersonGet handles HTTP/GET for Person.
func handlePersonGet(c persondao.PersonClient, w http.ResponseWriter, r *http.Request) {
...
// handlePersonPost handles HTTP/POST for Person.
func handlePersonPost(c persondao.PersonClient, w http.ResponseWriter, r *http.Request) {
	val, err := readPersonFromRequest(r)
	if err != nil {
...
// handlePersonDelete handles HTTP/DELETE for Person.
func handlePersonDelete(c persondao.PersonClient, w http.ResponseWriter, r *http.Request) {
	val, err := readPersonFromRequest(r)
	if err != nil {
...
// handlePersonPut handles HTTP/PUT for Person.
func handlePersonPut(c persondao.PersonClient, w http.ResponseWriter, r *http.Request) {
	val, err := readPersonFromRequest(r)
	if err != nil {
...
// handlePersonPatch handles HTTP/PATCH for Person.
func handlePersonPatch(c persondao.PersonClient, w http.ResponseWriter, r *http.Request) {
...
// readPersonFromRequest reads a Person from the given request. It returns an
// error if a Person couldn't be read.
func readPersonFromRequest(r *http.Request) (*data.Person, error) {
